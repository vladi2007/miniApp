...
<template>
  <div :class="$style.question">
    ...
    <div :class="$style.question__list">
      <div v-for="answer in answers" v-if="stage === 'discussion'" class="question_answer text_answer"
        :class="[$style.question__answer]">
        <span :class="$style.question__answer_text">{{ answer.text }}</span>

        <span v-if="isDiscussion" :class="[$style.question__answer__percent]">
          {{ answer.percentage }}%
        </span>
      </div>
    </div>
...
.question {
  ...
  &__list {
    ...
    &>div {
      ...
      border: calc((2 /832) * 100dvh) solid #6AB23D61;
