...
// список интерактивов
export const getInteractivities = async (
  filter: InteractivitiesListFilter,
  to_number: string,
  from_number: string
): Promise<InteractivitiesList> =>
  safeRequest(async () => {
...
export const getInteractive = async (interactive_id: number): Promise<Interactive> =>
  safeRequest(async () => {
    const { $api } = useNuxtApp()
    const res = await $api.get(`/interactivities/${interactive_id}`)
    return res.data
  })
...
export const postInteractive = async (formData: FormData): Promise<{ interactive_id: number }> =>
  safeRequest(async () => {
    const { $api } = useNuxtApp()
    const res = await $api.post(`/interactivities/`, formData, {
...
export const patchInteractive = async (formData: FormData, interactive_id: number): Promise<{ interactive_id: number }> =>
  safeRequest(async () => {
    const { $api } = useNuxtApp()
    const res = await $api.patch(`/interactivities/${interactive_id}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    return res.data
  })
...
// получить состояние интерактива
export const getInteractiveState = async (interactive_id: number): Promise<InteractiveState> =>
  safeRequest(async () => {
    const { $api } = useNuxtApp()
    const res = await $api.get(`/interactivities/is_running/${interactive_id}`)
    return res.data
  })
...
export const getInteractiveLeaderBoard = async (interactive_id: number): Promise<InteractiveLeaderBoard> =>
  safeRequest(async () => {
    const { $api } = useNuxtApp()
    const res = await $api.get(`/interactivities/end/${interactive_id}`)
    return res.data
  })